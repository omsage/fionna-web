...
const initTerminalWebsocket = () => {
  ...
  terminalWebsocket = new WebSocket(
    WSUri + "android/terminal?udid=" + selectDeviceUdid.value
  );
...
const getAppList = () => {
  axios
    .get("/android/app/list", { params: { udid: selectDeviceUdid.value } })
    .then((resp) => {
    ...
    >
      ...
      >
        <el-card
          ...
          v-show="0 === showCardMode"
          v-loading="udidListLoading"
        >
          <el-col
            v-show="serialInfoList.length !== 0"
          ...
          >
            <el-tooltip
              ...
              effect="dark"
              :content="
                $t('androidRemoteTS.product') +
                ':' +
                item.productDevice +
                ' ' +
                $t('androidRemoteTS.model') +
                ':' +
                item.model
              "
              placement="top"
            >
              <el-radio
                v-model="selectDeviceUdid"
                @input="selectGroupDevices"
                :label="item.udid"
                border
              >
