      ...
      <template #header>
        <div>
          ...
          <el-button
            ...
            @click="deleteReport"
            >{{ $t("report.delete") }}</el-button
          >
          <el-button
            style="float: right; margin-right: 10px"
            @click="getComparison"
            >{{ $t("report.comparison") }}</el-button
          >
        </div>
      </template>
...
const deleteReport = () => {
  ElMessageBox.confirm($t("report.deleteMess"), $t("elements.warn"), {
...
const selectReportList = (device, isCheckbox) => {
  if (isCheckbox) {
    ...
    selectUUIDList.value = selectUUIDList.value.filter(
      (item) => item !== device.uuid
    );
